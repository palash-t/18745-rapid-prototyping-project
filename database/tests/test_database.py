...
from database.database import get_db, \
							  ...
							  find_personal_check_in_by_patient_id_and_category, \
							  query_personal_check_in_by_time
...
def test_find_all_personal_check_in(database):
	results = find_all_personal_check_in(database)
	assert results is not None
	for result in results:
		assert result['id'] is not None
		assert result['patient_id'] is not None
...
def test_insert_personal_check_in(database):
	patient_id = uuid.uuid4()
	category = "instrument"
	value = "guitar"

	assert insert_personal_check_in(database, patient_id, category, value) is True

	results = find_personal_check_in_by_patient_id_and_category(database, patient_id, category)
	assert results is not None
	for result in results:
...
def test_insert_many_personal_check_ins(database, many_personal_check_ins):

	patient_id = many_personal_check_ins[1]["patient_id"]
	category = many_personal_check_ins[1]["category"]
	value = many_personal_check_ins[1]["value"]
	
	num_personal_check_ins = len(find_all_personal_check_in(database))

	num_added_personal_check_ins = len(many_personal_check_ins)

	assert insert_many_personal_check_ins(database, many_personal_check_ins) is True

	new_num_personal_check_ins = len(find_all_personal_check_in(database))

	assert new_num_personal_check_ins == num_personal_check_ins + num_added_personal_check_ins
...
def test_find_personal_check_in_by_patient_id_and_category(database, personal_check_in_patient_id, category):
	results = find_personal_check_in_by_patient_id_and_category(database, personal_check_in_patient_id, category)
	assert results is not None
	for result in results:
		assert result['patient_id'] is not None
		assert result['category'] is not None
...
def test_query_personal_check_in_by_time(database):
	...
	results = query_personal_check_in_by_time(database, start_time, end_time)
	assert results is not None
